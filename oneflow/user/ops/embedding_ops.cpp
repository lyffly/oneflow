...
/* static */ Maybe<void> EmbeddingLookupPlaceholderOp::ModifyInputArg(
  ...
  //user_op::InputArgModifier* var = GetInputArgModifierFn("var", 0);
  //CHECK_OR_RETURN(var != nullptr);
    ...
    .SetGenBackwardOpConfFn([](const user_op::UserOpWrapper& op,
      ...
      user_op::UserOpConfWrapper grad_op =
          builder.Op("embedding_update_placeholder")
              ...
              .Build();
      AddOp(grad_op);
      if(op.NeedGenGradTensor4OpInput("var", 0)) {
        user_op::UserOpConfWrapperBuilder var_grad_builder(op.op_name() + "_var");
        user_op::UserOpConfWrapper var_grad_op =
            var_grad_builder.Op("zero_like")
                .Input("like", op.input("var", 0))
                .Output("out")
                .Build();
        AddOp(var_grad_op);
        op.BindGradTensorWithOpInput(var_grad_op.output("out", 0), "var", 0);
      }
      return Maybe<void>::Ok();
    });
